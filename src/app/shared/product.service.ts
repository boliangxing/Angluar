...
export class ProductService {
  ...
  getbuyForProductId ( id: number): Observable<Comment[]> {
    return this.http.get('/api/buy?id=' + id + '/')
      .map(res => res.json() );
  ...
  search(params: ProductSearchParams): Observable<Product[]> {
    console.log(this.http.get('/api/productSearch?', {search : 'title=' + params.title + '&price=' + params.price + '&category=' + params.category }
    )
      .map(res => res.json()));
